...
fn _compile(ast: &Term, c: &mut Env) {
    match ast {
        ...
        Term::Rule(box Term::Ident(q1), box Term::Symbol(s), op, box Term::Ident(q2)) => {
            ...
            c.push_rule(q1.to_string(), s.clone(), op[0].clone(), op[1].clone(), q2.to_string());
        },
...
impl Env {
    fn new() -> Env {
        Env {
            target: vec!(), config: vec!(), symbol: vec!(ASTSym::Blank)
        }
    }
    ...
    fn push_rule(&mut self, q1: String, s: ASTSym, s1: Term, s2: Term, q2: String) {
        match (s1, s2) {
            (Term::Exec(o1), Term::Exec(o2)) => {
                ...
                self.target.push(
                    Rule {mc: rq1, sym: rs, op: ro, fc: rq2}
                )
            },
            _ => unreachable!(),
        }
    ...
    fn eff(&mut self, p: Step) -> Sym {
        match p {
            Step::Effect(k) => {
                match k {
                    Kind::Print(box Term::Symbol(s)) => {
                        match s {
                            ASTSym::Blank => {
                                Sym::S(self.index_symbol(ASTSym::Blank))
                            },
                            ASTSym::String(_) => {
                                let s1 = s.clone();
                                self.push_symbol(s);
